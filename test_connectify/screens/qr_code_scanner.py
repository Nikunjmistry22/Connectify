...
class QRScannerScreen(Screen):
    def __init__(self, **kwargs):
        ...
        # Add the back button to the text layout
    ...
    def scan_qr_code(self, dt):
        ...
        if decoded_objects:
            ...
            # print(lines)
            new_message_lines = []
            datetime_string = lines[1].split(': ')[1]  # Extract the datetime string part

            # Define the format of your datetime string
            datetime_format = "%Y-%m-%d %H:%M:%S"  # Replace this with your actual datetime format

            # Convert the datetime string to a datetime object
            last_active = datetime.strptime(datetime_string, datetime_format)
            current = datetime.now()

            # Convert the datetime object to a string
            current_datetime_str = current.strftime("%Y-%m-%d %H:%M:%S")

            # Define the format of your datetime string
            datetime_format = "%Y-%m-%d %H:%M:%S"

            # Convert the datetime string back to a datetime object
            converted_datetime = datetime.strptime(current_datetime_str, datetime_format)

            if last_active<=converted_datetime:
                first_part='Validity of this qr code is expired!'
                validity = Popup(
                    title='Qr Validity',
                    content=Label(text=f"{first_part}\n Please Restart the app"),
                    size_hint=(None, None),
                    size=(350, 150),
                )
                validity.open()
