...
class UserProfileScreen(Screen):
    def __init__(self, **kwargs):
        ...
        update_time_validity_button = Button(text='Refresh QR Code Validity', on_release=self.update_time_validity,
        ...
        profile_layout.add_widget(update_time_validity_button)
    ...
    def update_time_validity(self, instance):
        # Create a popup for user input
        content = BoxLayout(orientation='vertical', spacing=5)
        confirm_button = Button(text='Confirm')
        confirm_button.size_hint = (None, None)
        confirm_button.size = (100, 44)
        cancel_button = Button(text='Cancel')
        cancel_button.size_hint = (None, None)
        cancel_button.size = (100, 44)
        content_buttons = BoxLayout(spacing=5)
        content_buttons.add_widget(confirm_button)
        content_buttons.add_widget(cancel_button)
        content.add_widget(content_buttons)

        popup = Popup(
            title='Update Time Validity',
            content=content,
            size_hint=(None, None),
            size=(350, 150),
            auto_dismiss=False
        ...
        def on_confirm_button(instance):
            ...
            # Define a time delta of 2 minutes
            two_minutes = timedelta(minutes=5)

            # Calculate the new datetime with 2 minutes added
            new_datetime = current_datetime + two_minutes

            # Format the datetimes as strings
            current_datetime_str = current_datetime.strftime("%Y-%m-%d %H:%M:%S")
            new_datetime_str = new_datetime.strftime("%Y-%m-%d %H:%M:%S")
            qr_data = f"doc: {current_datetime_str}\nlad: {new_datetime_str}"
            qr.add_data(qr_data)
            ...
            try:
                ...
                if result:
                    self.db_message=result[0]
                else:   self.db_message="No Qr Code Generated Yet"
            except sqlite3.Error as e:
                print(f"SQLite Error: {e}")
            finally:
                conn.close()
            qr.add_data(self.db_message)
            qr.make(fit=True)
            ...
            success = Popup(
                title='Success',
                content=Label(text='Successfully Updated QR Code'),
                size_hint=(None, None),
                size=(350, 150),
            )
            success.open()
